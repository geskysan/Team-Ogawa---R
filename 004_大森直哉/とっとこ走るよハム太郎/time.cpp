...
void InitTimer(void)
{
	...
	{
		SetTimer(D3DXVECTOR3(550 + 50 * x, 10, 0));	//タイマー（３桁）
	}
...
void UpdateTimer(void)
{
	...
	if (iTime <= 0)
	{
		...
		for (nCutVtx = 0; nCutVtx < MAX_DIGIT; nCutVtx++) {
			...
			float fTime = iTime % iPow / (iPow / 10);
...
void MakeVertexTimer(LPDIRECT3DDEVICE9 pDevice)
{
	...
	for (nCutVtx = 0; nCutVtx < MAX_DIGIT; nCutVtx++) {
		...
		pVtx[1].pos.x = g_aTimer[nCutVtx].pos.x + 50;
		pVtx[2].pos.x = g_aTimer[nCutVtx].pos.x;
		pVtx[3].pos.x = g_aTimer[nCutVtx].pos.x + 50;

		pVtx[0].pos.y = g_aTimer[nCutVtx].pos.y;
		pVtx[1].pos.y = g_aTimer[nCutVtx].pos.y;
		pVtx[2].pos.y = g_aTimer[nCutVtx].pos.y + 50;
		pVtx[3].pos.y = g_aTimer[nCutVtx].pos.y + 50;
		...
		//int iPow = powf(10, MAX_DIGIT - nCutVtx);
		//float fTime = iTime % iPow / (iPow / 10);
		//pVtx[0].tex = D3DXVECTOR2((0 + g_aTimer[MAX_DIGIT].g_nPatternAnim)*1.0 / (MAX_PATTEN), 1.0*(g_aTimer[MAX_DIGIT].g_nPatternAnim / 5));
		//pVtx[1].tex = D3DXVECTOR2((1 + g_aTimer[MAX_DIGIT].g_nPatternAnim)*1.0 / (MAX_PATTEN), 1.0*(g_aTimer[MAX_DIGIT].g_nPatternAnim / 5));//少数のため1.0をかける
		//pVtx[2].tex = D3DXVECTOR2((0 + g_aTimer[MAX_DIGIT].g_nPatternAnim)*1.0 / (MAX_PATTEN), 1.0*(g_aTimer[MAX_DIGIT].g_nPatternAnim / 5 + 1));//整数のため１をかける
		//pVtx[3].tex = D3DXVECTOR2((1 + g_aTimer[MAX_DIGIT].g_nPatternAnim)*1.0 / (MAX_PATTEN), 1.0*(g_aTimer[MAX_DIGIT].g_nPatternAnim / 5 + 1));
